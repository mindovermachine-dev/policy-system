...
- Under `-v`/`--verbose`, also print the failure site (`🔦 @ {file}:{line}`, the innermost frame of the caught error's traceback) to stderr after the formatted message — a lightweight diagnostic short of a full traceback.
