...
| Technical Debt | Query Engine's guard/execution logic (`ExecuteCypherQuery`) is implemented but misplaced in `mcp_interface/cypher_cli.py` instead of `query_engine/`, with MCP Interface invoking it out-of-process rather than in-process — already flagged in Container Architecture but not yet scheduled for relocation | Relocate before additional functionality is built on top of the current (wrong) module boundary |
