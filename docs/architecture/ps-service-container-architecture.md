...
- **Transport:** as of issue #39, a network-reachable Streamable HTTP transport is shipped and mounted at `/mcp` inside the same FastAPI app that already serves the REST entry-point layer (`ps_service/api/`) — same process, same container, same port, not a separate network service, exactly as this section previously called for. MCP's stdio transport (a locally-spawned child process) remains available unchanged alongside it, for the fully-local Claude Desktop setup validated earlier; the two transports are both live at once, not mutually exclusive — a client picks whichever fits its deployment.
...
| `ps-service/src/ps_service/mcp_interface/mcp_server.py` | MCP stdio server: `HandleMcpToolCall` (in-process call to Query Engine's `execute_cypher_query`, per-call `run_id` binding) and `GetDomainConcepts` (MCP resource `psdomain://concepts`) | `HandleMcpToolCall`, `GetDomainConcepts` |
