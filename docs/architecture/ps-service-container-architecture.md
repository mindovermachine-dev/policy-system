...
- How far a Domain Mapping Adapter extracts down the compliance spine is adapter-specific: the Cellar/ELI adapter (`source_type: external`) stops at Capability. For `source_type: internal`, `DeriveGovernanceArtifacts` continues past Capability through Policy/Standard/Control via `GOVERNED_BY`/`SUPPORTED_BY`/`IMPLEMENTED_BY`, gated on `RegulatoryInstrument.source_type == internal` — it reads the baseline graph directly, as `DeriveObligationsAndCapabilities` does, not via a paired Domain Mapping Adapter (no such object exists for governance derivation; see [`ps-domain-concepts.md`](../artifacts/ps-domain-concepts.md#document-purpose) for the canonical dual-origin model this reflects).
...
- Exposed via `POST /restorations` (REST), never called directly by `ps-cli` — `ps-cli restore instrument` reads the artifact locally from the configured curated-repo location and uploads it; PS Service does the actual FalkorDB work, keeping `ps-cli` decoupled from `falkordb`/`redis` entirely.
...
- **Authentication is explicitly open, not resolved:** deferred while PS Service and its clients run on the same local machine (one trust boundary already covers both), but this must be decided before any remote deployment. Query Engine's query cost is now bounded (issue #38 — a `PS_QUERY_TIMEOUT_MS` execution timeout and a `PS_QUERY_ROW_CAP` result-size cap, see [Query Engine](#query-engine)), but authentication remains the open half of this risk combination: a network-reachable MCP Interface without auth is still an unauthenticated Cypher endpoint — do not treat the current "No (deferred)" as safe-by-default once remote hosting is in scope. PS-Cli's REST-side auth direction is now decided (OIDC via Auth0, OAuth 2.0 Device Authorization Grant, individual-operator identity — see `ps-solution-architecture.md`'s Risks & Concerns) and is a candidate mechanism for this boundary too, but whether MCP Interface shares that identity provider or needs its own is not yet decided. The one explicit exception is the opt-in local-test bypass (`PS_SERVICE_LOCAL_TEST_BYPASS`, issue #67): enforced loopback-only by refusal to bind on any other host, and announced with a structured warning on every process start. It does not resolve this open item and must not be read as the deferred real auth having landed — issue #39 ships the Streamable HTTP transport itself with no additional auth beyond this same bypass; real per-user auth for it stays open, tracked as issue #39's Group 3 / issue #65.

  **Recorded decision — Auth0 tenant sharing between REST and MCP Interface** (documentation only; REST's own resource-server/token-validation side is not yet implemented either, per this bullet's own caveat above and `ps-solution-architecture.md`'s risk-register row):

  > **Recommendation: share one Auth0 tenant across REST and MCP Interface, as two separate Auth0 application/client registrations against that one tenant** (one OAuth 2.0 Device Authorization Grant client for PS-Cli, already decided; one OAuth client for MCP's own client-side authorization flow, per the MCP Authorization spec — the SDK's `create_auth_routes`/`create_protected_resource_routes`, observed directly in `Server.streamable_http_app`'s source, already implement the resource-server/authorization-server metadata endpoints that flow needs). Rationale: both REST and MCP Interface (as of this issue) run in the exact same process, serve the exact same operator population against the exact same graph, and both need the same "individual-operator identity, not a shared per-deployment secret" property the risk register already commits to for REST. One tenant means one identity/user directory and one audit-trail namespace (the `principal` recorded on both REST and `query_engine` log entries refers to the same underlying operator either way) instead of two independent credential-issuance systems for what is one trust boundary. This also keeps AC-BI-010 ("no bespoke client code") achievable for free: an MCP client authenticates via its own built-in OAuth flow against Auth0-as-authorization-server, using the SDK's already-implemented auth routes — no custom login code needed in `ps_service` itself, only a `TokenVerifier` implementation and `auth`/`auth_server_provider` configuration when #65 settles the mechanism. This is a recommendation for #65 to formally decide, not a decision this issue enforces in code — Group 3 (AC-BI-007..011) stays fully deferred.
