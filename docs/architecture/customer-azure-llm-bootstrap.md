...
- **Quota exhaustion has no automated remedy** — an evaluator hitting subscription-wide quota limits must request an increase from Azure directly, or free capacity a prior deploy's soft-deleted account is still reserving (the script prints the exact `az cognitiveservices account delete`/`purge` commands for its own deterministic account name when this failure fires, same pattern as the Key Vault soft-delete purge above); the script can only detect and report the failure, not resolve a genuine subscription-wide shortfall.
