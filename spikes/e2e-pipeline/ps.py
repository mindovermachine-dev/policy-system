...
# (`shell(spikes/e2e-pipeline/ps.py:*)`) rather than the interpreter,
...
"""PS CLI -- minimal command surface for the e2e-pipeline spike.

Forked from spikes/cli-tool-semantics/ps.py (README.md D2 -- no live
cross-spike import). query_mechanism_v1.py and catalog.py are forked
alongside it as this file's real dependency closure -- they were the only
two query1/query2 modules ps.py actually imports (verified: catalog.py's
own sys.path.insert toward query1 was dead code, nothing in it imports
query_mechanism_v1). Everything else in query1/query2 (experiments, docs,
resolver.py) stays behind in those spikes, unforked, because ps.py never
touched it.

This CLI does not reimplement any query logic -- it wraps the two
mechanisms already proven as deterministic Python libraries in the query
spikes:

  - ``ps query template`` wraps query_mechanism_v1.py (the v1
    parameterized-template NL->Cypher router).
  - ``ps query catalog``  wraps catalog.py (Candidate D, the
