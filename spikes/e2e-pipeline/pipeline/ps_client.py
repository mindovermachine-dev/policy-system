...
"""Thin wrapper around this spike's own ps.py (forked from
spikes/cli-tool-semantics/ps.py, README.md D2 -- no live cross-spike
import) -- Stage 4's graph access. Reuses the CLI's proven deterministic
query surface rather than reimplementing query logic (README.md, "Other
prior findings this design reuses directly"). Every Stage 4 sub-check
below independently re-queries through here -- never reuses whatever
query produced the original answer (the lesson of SWE-M1/RM-E1's
query-construction misses, see README.md Stage 4).
...
_PS_PY = Path(__file__).resolve().parent.parent / "ps.py"
