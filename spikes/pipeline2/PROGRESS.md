...
Run `policy-question` v0.2.0 (narrowing + freehand retrieval + answer
construction) against a few more questions to see whether this increment
pulls its weight on its own, per D13's incremental-validation discipline —
then build the fitness-authoring skill (Setup step 2, second half),
authored against [RUBRIC.md](./RUBRIC.md), taking the skill's refined
