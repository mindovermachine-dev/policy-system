...
# Dev-set runner for the cli-tool-semantics spike.
# Runs each question from skill-transfer's dev question catalog (reused
# verbatim, per NEXT-ACTIONS.md thread 4) as a fresh headless Copilot CLI
# session (kimi-k3), scoped to the `ps` CLI only -- no raw redis-cli/Cypher --
...
HARNESS_PREFIX='You have access to a `ps` CLI wrapping a deterministic policy-graph query surface. Query it ONLY by running commands of the form: spikes/cli-tool-semantics/ps.py <args> -- no pipes, no chained commands, no other shell tools, no raw redis-cli/Cypher/python3. Run `spikes/cli-tool-semantics/ps.py templates` or `--help` on any subcommand if unsure of the command surface. If a command fails, report the error; do not fall back to reading files. For reference, today is 2026-08-01. Question:'
...
# Small, deliberately mixed subset for --smoke: two template-router hits
# (SA-E1, PM-E1), one template hit shaped as a chain trace (AU-M1), and one
# question with no template match that should fall through to `ps cypher`
# (SEC-M1) -- covers command-selection across the full order in SKILL.md.
SMOKE_IDS=(SA-E1 PM-E1 AU-M1 SEC-M1)

if (( SMOKE )); then
  ORDERED_IDS=("${SMOKE_IDS[@]}")
else
