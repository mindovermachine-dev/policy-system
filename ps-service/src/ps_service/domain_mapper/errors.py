...
class DomainMapperExtractionError(Exception):
    """A single unit's LLM-driven extraction call could not be turned into
    valid `RequirementCandidate`s — malformed/unparseable JSON, or a
...
class DomainMapperDerivationError(Exception):
    ...
    Raised by `derivation.py`.
    """


class DomainMapperPersistenceError(Exception):
    """A FalkorDB write for the baseline graph could not be completed
    safely — e.g. a Requirement node about to be persisted references a
    Role node not present in the same write call (PLAN_REVIEWED.md §5.4's
    B3 fix (a)).
...
class DomainMapperConfigurationError(Exception):
    """Domain Mapper's FalkorDB connection could not be established from
    the resolved `ServiceConfig` (unreachable host/port, or the connection
    could not be validated).

    Raised by `falkordb_client.connect`/`connect_from_config`.
    """
