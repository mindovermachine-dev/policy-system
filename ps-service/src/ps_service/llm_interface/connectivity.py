...
def check_connectivity(config: ServiceConfig) -> None:
    """Confirm the configured LLM Provider is reachable for both completion
    and embedding. LLM Interface is a hard-required dependency regardless of
    `ServiceConfig`'s own optionality for `llm_interface_model`/
    `llm_interface_embed_model` (both `str | None`, unset in configurations
    that never call this function) — raises `LlmProviderError` if either
    model is unconfigured, without ever making a real call, distinct from a
    configured-but-unreachable model.

    `route_completion`/`route_embedding` already record the outcome in
    `ps_service.dependency_health` themselves on every call (this probe
    included), so a real subsequent failure/success during normal operation
    continues to update the same live signal this startup check seeds.
    """
