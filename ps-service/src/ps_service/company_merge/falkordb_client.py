...
class GraphHandle(Protocol):
    ...
    The `query()` call surface this component's graph reader/dedup/writer/merge
...
def connect_from_config(config: ServiceConfig) -> FalkorDB:
    ...
    Mirrors `ps_service.domain_mapper.falkordb_client.connect_from_config`
...
def select_graph(db: FalkorDB, name: str) -> GraphHandle:
    ...
    `ps_service.company_merge` never import `falkordb.Graph` directly. `name`
    is expected to already be the final graph name (e.g. the result of
    `single_tenant_graph_name()`, or a `{short}_baseline` name built
...
def single_tenant_graph_name() -> str:
    ...
    `ps_service.mcp_interface.mcp_server`'s `DEFAULT_GRAPH` convention exactly
    (verified directly against that module's own source, not assumed by
    analogy) -- this is the one graph name shared across the whole system for
    read-only query access and Company Merge's own add/merge-only writes.
    """
