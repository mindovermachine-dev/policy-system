...
class McpGraphUnavailableError(Exception):
    """The FalkorDB-backed policy graph could not be acquired when handling an MCP
    tool call -- unreachable, refused, a driver I/O failure in the eager client
    constructor, or an invalid PS_FALKORDB_* configuration. The message is
    deliberately generic: host / port / driver / env-var detail must not cross the
    MCP boundary (L2 MCP Interface Patterns)."""


class McpResourceUnavailableError(Exception):
    """A backing file for an MCP resource (ps-domain-concepts.md) is missing or
    unreadable. Raised by the resource read function; the mcp SDK surfaces a clean
