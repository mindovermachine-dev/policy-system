...
(persistence + configuration). Adapter-level fetch/parse failures have their
own types — see `ps_service.ingestion.adapters.errors`.
"""
...
class IngestionConfigurationError(Exception):
    """Ingestion's FalkorDB connection could not be established from the
    resolved `ServiceConfig` (unreachable host/port, or the connection
    could not be validated) — raised by `falkordb_client.connect`/
    `connect_from_config`.
    """
