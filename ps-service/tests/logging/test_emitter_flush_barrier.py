...
def test_flush_when_entries_queued_then_all_written_before_return(make_emitter, read_lines) -> None:
...
def test_double_flush_does_not_hang(make_emitter) -> None:
...
def test_stop_when_called_twice_then_is_idempotent(make_emitter) -> None:
    emitter, _ = make_emitter()
    emitter.stop(timeout=2.0)
    emitter.stop(timeout=2.0)  # no-op, must not raise or hang


def test_flush_when_writer_stopped_then_raises_logging_lifecycle_error(make_emitter) -> None:
