...
def test_emit_when_target_unwritable_then_falls_back_to_stderr_via_capsys(
    ...
    emitter = LogEmitter(EmitterConfig(log_path=log_path))  # fallback=None -> resolves sys.stderr dynamically
    ...
    captured = capsys.readouterr()  # N1: read only AFTER flush() — ordering is enforced, not assumed
...
def test_emit_when_extra_not_json_serializable_then_falls_back_and_writer_survives(
    tmp_path: Path, read_lines
) -> None:
    log_path = tmp_path / "test.jsonl"
    emitter = LogEmitter(EmitterConfig(log_path=log_path))

    bad_entry = LogEntry(component="ac3", action="bad", extra=(("ts", object()),))  # not JSON-serializable
    good_entry = LogEntry(component="ac3", action="good")

    emitter.emit(bad_entry)
    emitter.emit(good_entry)
    emitter.flush()  # both handled — if the writer had died, this would hang (proves M6+M3 together)

    lines = read_lines(log_path)
    assert any(line.get("action") == "good" for line in lines)  # writer kept running after the bad entry


def test_write_failure_when_target_missing_parent_then_log_file_never_created(tmp_path: Path) -> None:
    from io import StringIO
