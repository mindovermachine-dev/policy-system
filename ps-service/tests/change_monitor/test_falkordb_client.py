"""Tests for `ps_service.change_monitor.falkordb_client`.

Mirrors `ps_service.company_merge.tests.test_falkordb_client`'s shape -- this
is a deliberate near-duplicate component (own copy, not a shared import; see
the module's own docstring), so its tests are a deliberate near-duplicate
too, plus a `native_graph_name` case (this component also drives the
`{short}_native` graph, mirroring `domain_mapper.falkordb_client`).
"""
...
from ps_service.change_monitor import falkordb_client as falkordb_client_module
...
def test_check_connectivity_raises_change_monitor_configuration_error_on_failure() -> None:
    probe = _FakeConnectivityProbeThatRaises()

    with pytest.raises(ChangeMonitorConfigurationError) as exc_info:
...
def test_check_connectivity_marks_falkordb_unhealthy_on_failure() -> None:
    ...
    with pytest.raises(ChangeMonitorConfigurationError):
        check_connectivity(probe, host="127.0.0.1", port=6379)
...
def test_connect_from_config_uses_env_supplied_host_and_port_not_hardcoded_defaults(
    ...
    env_port = 7000
    assert env_host != ServiceConfig.falkordb_host
    assert env_port != ServiceConfig.falkordb_port
