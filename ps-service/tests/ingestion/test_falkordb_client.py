"""Tests for `ps_service.ingestion.falkordb_client`."""
...
def test_check_connectivity_raises_falkordb_connection_error_on_failure() -> None:
...
def test_connect_from_config_uses_env_supplied_host_and_port_not_hardcoded_defaults(
    ...
    """S4 proof (PLAN_REVIEWED.md §6.4/Increment 12): `connect_from_config`
    threads `ServiceConfig.falkordb_host`/`falkordb_port` — resolved from
    `PS_FALKORDB_HOST`/`PS_FALKORDB_PORT` by `load_config()` — into
    `connect()`, not `config_module._DEFAULT_FALKORDB_HOST`/
    `_DEFAULT_FALKORDB_PORT`. Mocks the module-level `connect` (the
    underlying call) to capture exactly what it was invoked with, so this
    fails if `connect_from_config` ever hardcodes a literal instead of
    reading `config`.
    """
