...
def tokens(cap: dict[str, Any]) -> set[str]:
    text = f"{cap['name']} {cap.get('description', '')}"
    words = re.sub(r"[^a-z0-9\s]", " ", text.lower()).split()
    return {w for w in words if w not in STOPWORDS}


def build_idf(cap_tokens: dict[str, set[str]]) -> dict[str, float]:
    n = len(cap_tokens)
...
def cosine(toks_a: set[str], toks_b: set[str], idf: dict[str, float]) -> float:
...
def fetch_capabilities(graph: _Graph) -> list[dict[str, Any]]:
    result = graph.query("MATCH (n:Capability) RETURN n")
    return [row[0].properties for row in result.result_set]


def fetch_source_regulations(graph: _Graph, cap_id: str) -> list[Any]:
    result = graph.query(
        """
        MATCH (reg:RegulatoryInstrument)-[:DEFINES]->(:Role)-[:HAS]->(:Obligation)
              -[:REQUIRES]->(c:Capability {id: $cap_id})
...
def print_pair_report(
    ...
    """Print the ranked candidate pairs (with their source regulations) for review."""
    reg_cache: dict[str, list[Any]] = {}

    def regs_for(cap_id: str) -> list[Any]:
        if cap_id not in reg_cache:
            reg_cache[cap_id] = fetch_source_regulations(graph, cap_id)
        return reg_cache[cap_id]

    for score, a_id, b_id in scored_pairs[:top]:
    ...
    if len(scored_pairs) > top:
        print(f"... {len(scored_pairs) - top} more pair(s) below the --top cutoff\n")
...
def main() -> int:
    ...
    if len(capabilities) < 2:
        print(
            f"Only {len(capabilities)} Capability node(s) in "
            f"'{args.graph_name}' -- nothing to compare."
        )
        return 0

    by_id: dict[str, dict[str, Any]] = {c["id"]: c for c in capabilities}
    cap_tokens: dict[str, set[str]] = {c["id"]: tokens(c) for c in capabilities}
    ...
    print(
        f"Graph: {args.graph_name}  |  {len(capabilities)} Capability nodes  "
    ...
    if not scored_pairs:
        print(
            "No candidate duplicates above threshold. Try lowering --threshold if you expect more."
        )
        return 0

    print_pair_report(graph, scored_pairs, by_id, args.top)
